...
inline static float interpolate(float alpha, float beta, float gamma, const float& z1, const float& z2, const float& z3, float weight)
{
    return (alpha * z1 + beta * z2 + gamma * z3) / weight;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    std::array<Vector4f, 3> v = {t.a(), t.b(), t.c()};
    
    // TODO : Find out the bounding box of current triangle.
    Vector4f a(v[0]), b(v[1]), c(v[2]);
    std::array<Vector4f, 3> screenPoints;
    std::transform(std::begin(v), std::end(v), screenPoints.begin(), [](auto& vec) {
        return Vector4f(vec.x(), vec.y(), 0.0f, 1.0f);
    });
